...
def read_database():
    try:
        database_df = pd.read_excel('extracts/GA_database.xlsx', sheet_name='Master Today')
        database_df.to_excel('extracts/GA_database.xlsx', sheet_name='Master Yesterday', engine='xlsxwriter')
    except:
        database_df = []
    return database_df
    
def request_ga(_client, property_id, start_date_input, end_date_input, flag=False):
...
def request_ga_events(_client, property_id, start_date_input, end_date_input):
    ...
    events_df = pd.pivot_table(events_df, index=['date', 'yearMonth', 'country', 'firstUserDefaultChannelGroup'],columns=['eventName'], values='keyEvents', aggfunc="sum")
...
def clean_df(df, flag=''):
    ...
    # df_final['averageSessionDuration'] = df_final['averageSessionDuration'].apply(lambda x: round(x,0)).astype('float')
    df_final['averageSessionDuration'] = df_final['averageSessionDuration'].astype('float') 
    df_final['SessionsDuration'] = df_final['averageSessionDuration'] * df_final['Sessions']
    ...
    for index, row in df_final.iterrows():
        ...
        if row['country'].lower().strip() in ('(not set)','not set',''):
            row['country'] = 'INCONNU'
    if flag=='pages':
        df_final = df_final.groupby(['yearMonth','country','pagePath','firstUserDefaultChannelGroup']
                                ).agg(activeUsers=('activeUsers','sum'), Sessions=('Sessions','sum'), engagedSessions=('engagedSessions','sum'),
                                    SessionsDuration=('SessionsDuration','sum'), screenPageViews=('screenPageViews','sum'),
                                ).sort_values('activeUsers', ascending=False
                                ).reset_index()
...
def clean_pages(pages_table):
    product_pages_final = pages_table.groupby('pagePath').agg(activeUsers=('activeUsers','sum'), Sessions=('Sessions','sum'),
                                                        engagedSessions=('engagedSessions','sum'), SessionsDuration=('SessionsDuration','sum'),
                                                        screenPageViews=('screenPageViews','sum'),
                                                ).sort_values('activeUsers', ascending=False
                                                ).reset_index()
    product_pages_final['product'] = product_pages_final['pagePath'].apply(lambda x: x.split('.')[0])
    product_pages_final['product'] = product_pages_final['product'].apply(lambda x: x.split('/')[-1])
    product_pages_final['product'] = product_pages_final['product'].apply(lambda x: x.replace('-',' ').replace('_',' ').replace(' de l a l','').replace(' d '," d'"))
    product_pages_final['product'] = product_pages_final['product'].apply(lambda x: re.sub(r'[0-9]','',x))
    product_pages_final['product'] = product_pages_final['product'].str.strip(' ')
    product_pages_final['product'] = product_pages_final['product'].str.capitalize()
    product_pages_final['avgSessionDuration'] = (product_pages_final['SessionsDuration'] / product_pages_final['Sessions']).map('{:.1f}'.format)
    # product_pages_final['avgSessionDuration'] = (product_pages_final['SessionsDuration']/product_pages_final['Sessions']).apply(lambda x: round(x,0)).astype('int')
    
    return product_pages_final
