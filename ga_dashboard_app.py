...
def request_data(_client, property_id, start_date_input, end_date_input, start_date_comparison, end_date_comparison):
    ...
    product_pages_final = clean_df(pages_df, flag='pages')
    product_pages_final = clean_pages(product_pages_final)
    ...
    # COMPARISON TABLE with events enrichment
        ...
        comp_df = clean_df(comp_df, flag='comp')
    ...
    # MERGING MASTER TABLE TO COMPARISON TABLE
        ...
        # PROACTIVE EXTRACT DATA TO EXCEL FILE
        df_final.to_excel('extracts/GA_database.xlsx', sheet_name='Master Today', engine='xlsxwriter')
        product_pages_final.to_excel('extracts/GA_database.xlsx', sheet_name='Products Today', engine='xlsxwriter')

        return df_final, product_pages_final
...
def initialize_filters(df_final, product_pages_final):
...
# FILTERS SIDEBAR -- PART 2
def apply_filters(df_final, product_pages_final):
    df_filtered_except_channels = df_final.copy()
    product_pages_filtered = product_pages_final.copy()
    ...
    if st.session_state['country_filter']:
        ...
        product_pages_filtered = product_pages_filtered[product_pages_filtered["country"].isin(st.session_state["country_filter"])]
    ...
    if st.session_state['yearMonth_filter']:
        ...
        product_pages_filtered = product_pages_filtered[product_pages_filtered["yearMonth"].isin(st.session_state["yearMonth_filter"])]
    ...
    if st.session_state['channel_filter']:
        ...
        product_pages_filtered = product_pages_filtered[product_pages_filtered["firstUserDefaultChannelGroup"].isin(st.session_state["channel_filter"])]
    else:
    ...
    return df_filtered, df_filtered_except_channels, product_pages_filtered

# FOR GRAPH 1
def build_funnel(df_filtered, df_filtered_except_channels, product_pages_filtered):
...
# FOR GRAPHS 2 & 3
# FOR TABLES 1 & 2
def build_yearMonth_agg(df_filtered, df_filtered_except_channels, product_pages_filtered):
    ...
    with st.expander("Comparaison versus l'Année Précédente", expanded=True):
        ...
        if selected_yearMonth_evol!=st.session_state["selected_yearMonth_evol"]:
            ...
            df_filtered, df_filtered_except_channels, product_pages_filtered = apply_filters(df_final, product_pages_final)
            # st.write(st.session_state["yearMonth_filter"])
            # st.rerun()
...
def build_yearMonth_area(df_filtered, df_filtered_except_channels, product_pages_filtered, yearMonth_agg):
    ## GRAPH 2
...
def build_yearMonth_kpis(df_filtered, df_filtered_except_channels, product_pages_filtered, yearMonth_agg):
    ## TABLE 2
...
# FOR GRAPH 3
def build_channel(df_filtered, df_filtered_except_channels, product_pages_filtered):
...
def build_countries(df_filtered, df_filtered_except_channels, product_pages_filtered):
...
def build_products(pages_final):
    ...
    product_pages_flag = st.toggle("Pages Produits (exclusion des pages de navigation)", value=True)
    # product_pages=pages_final.copy()
    product_pages = pages_final[(~pages_final['pagePath'].isin(pages_exclues))]
    if product_pages_flag:
        chosen_variable = 'Pages Produits Vus'
        # product_pages['product']  = product_pages['pagePath'].str.replace('-',' ').replace('.',' ')
        # product_pages['product'] = product_pages['product'].str.strip(' ')
        product_pages['output'] = product_pages['product'].values.astype(str)
    else:
        chosen_variable = 'Pages URL Visitées'
        product_pages['output'] = product_pages['pagePath'].values.astype(str)
    product_pages['output'].dropna(inplace=True)
        
    top_results_label = min(st.session_state["top_results"], len(product_pages))
    product_pages.reset_index(inplace=True)
    product_pages.index += 1
    
    product_pages['avgPageViews'] = (product_pages['screenPageViews'] / product_pages['activeUsers']).round(1)
    product_pages.drop(columns=['SessionsDuration','Sessions','screenPageViews'], inplace=True)
    if len(product_pages) > 0:
        st.dataframe(product_pages[0:top_results_label],column_order=["output", "activeUsers", "engagedSessions", "avgSessionDuration",'avgPageViews'], row_height=35, height=35*(top_results_label+1), #height=37*top_results_label,
                    on_select="rerun",
                    column_config={
                "output": st.column_config.TextColumn(chosen_variable),
                "activeUsers": st.column_config.ProgressColumn("Visiteurs",format="localized",min_value=0,max_value=max(product_pages['activeUsers'][0:top_results_label])),
                "engagedSessions": st.column_config.ProgressColumn("Sessions Engagées",format="localized",min_value=0,max_value=max(product_pages['engagedSessions'][0:top_results_label])),
                "avgSessionDuration": st.column_config.ProgressColumn("Durée Moy. Session (s)",format="localized",min_value=0,max_value=max(product_pages['avgSessionDuration'][0:top_results_label])),
                "avgPageViews": st.column_config.ProgressColumn("Moy. Pages Vues",format="localized",min_value=0,max_value=max(product_pages['avgPageViews'][0:top_results_label])),
                },)

# MAIN APP
def main(df_filtered, df_filtered_except_channels, product_pages_filtered):
    # GRAPH 1 -- FUNNEL MARKETING
    build_funnel(df_filtered, df_filtered_except_channels, product_pages_filtered)

    ## TABLE 1 - Comparaison versus l'Année Précédente
    yearMonth_agg = build_yearMonth_agg(df_filtered, df_filtered_except_channels, product_pages_filtered)
    st.write("##")
    
    # GRAPH 2 - Visiteurs vs Sessions
    build_yearMonth_area(df_filtered, df_filtered_except_channels, product_pages_filtered, yearMonth_agg)
    st.write("##")
    
    ## TABLE 2 - Principaux KPIs de Performance
    build_yearMonth_kpis(df_filtered, df_filtered_except_channels, product_pages_filtered, yearMonth_agg)
    st.write("##")
    
    ## GRAPHS 3, 4
    build_channel(df_filtered, df_filtered_except_channels, product_pages_filtered)
    st.write("##")
    
    ## TABLE 3
    build_countries(df_filtered, df_filtered_except_channels, product_pages_filtered)

    ## TABLE 4
    build_products(product_pages_final)
...
if __name__ == "__main__":
    ...
    # FILTERS
    df_final, product_pages_final = initialize_filters(df, pages_df)
    df_filtered, df_filtered_except_channels, product_pages_filtered = apply_filters(df_final, product_pages_final)
    
    # USER DOWNLOAD of non-filtered data
    today_str = datetime.now().strftime('%Y-%m-%d at %H.%M.%S')
    
    master_to_download = export_to_excel(df_final)
    st.sidebar.download_button(label="Master Data", data=master_to_download, mime="text/csv", icon=":material/download:", file_name=f'{today_str} - Master Source.xlsx')
    
    products_to_download = export_to_excel(product_pages_final)
    st.sidebar.download_button(label="Products", data=products_to_download, mime="text/csv", icon=":material/download:", file_name=f'{today_str} - Products Source.xlsx')
    st.sidebar.caption("Attention : données seulement à partir de JUIN 2023.")
    
    # ANALYSES & VISUALIZATIONS
    main(df_filtered, df_filtered_except_channels, product_pages_filtered)
